...
#include <QString>
using namespace std;
commandr::commandr(QObject *parent)
    : QObject{parent}
{}
QMap<int, QString> commandr::getServiceNames() {
    return {
        { 1, "tcpmux" },
        { 3, "compressnet" },
        { 4, "unknown" },
...
void commandr::scanPorts(const QString &ip) {
    ...
    totalTimer.start();
    for (auto it = portServices.begin(); it != portServices.end(); ++it) {
        int port = it.key();
